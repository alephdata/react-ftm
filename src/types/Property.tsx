...
class PropertyValues extends React.PureComponent<IPropertyValuesProps, IPropertyValuesState> {
  ...
  render() {
    ...
    const vals = ensureArray((truncate && !truncateShowAll) ? values.slice(0, truncate) : values).map((value: any) => (
      <PropertyValue
    ...
    if (!vals.length) {
      ...
      content = vals.map((val: any, i: number) => <span key={i} style={{ display: 'block' }}>{val}</span>);
    } else {
