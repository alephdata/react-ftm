...
class TableEditorBase extends React.Component<ITableEditorProps, ITableEditorState> {
  ...
  componentDidUpdate(prevProps: ITableEditorProps, prevState: ITableEditorState) {
    ...
    const sortChanged =
      prevProps.sort?.field !== sort?.field || prevProps.sort?.direction !== sort?.direction;
  ...
  findEntityRange = (i0: number, i1: number) => {
    ...
    return this.state.entityRows.slice(start, end + 1).map((row) => row[1]?.data?.entity?.id);
  };
  ...
  renderAddButton = () => {
    ...
    return (
      <Tooltip
        content={intl.formatMessage(messages[showTopAddRow ? 'remove' : 'add'], {
          schema: schema.label,
        })}
      >
        <Button
  ...
  renderEntityLinkButton = ({ entity }: { entity: FTMEntity }) => {
    ...
    return <Button minimal small icon="fullscreen" onClick={() => visitEntity(entity)} />;
  };

  renderSkeleton = () => {
    const skeletonLength = 15;
    return <span className={Classes.SKELETON}>{'-'.repeat(skeletonLength)}</span>;
  };
  ...
  handleExistingRow = (
    changes: Datasheet.CellsChangedArgs<CellData, any> | Datasheet.CellAdditionsArgs<CellData>
  ) => {
