...
export class GraphLayout {
  ...
  getSelectedGroupings(): Grouping[] {
    return this.getGroupings().filter(
      (grouping) => grouping.id !== 'selectedArea' && this.isGroupingSelected(grouping)
    );
  }

  getSelectedEntityIds() {
    return this.getEntityIds(...this.getSelectedVertices(), ...this.getSelectedEdges());
  }
      ...
      if (entity.schema.edge) {
        ...
        entity.getProperty(sourceProperty).forEach((source) => {
          entity.getProperty(targetProperty).forEach((target) => {
            const sourceEntity =
              typeof source === 'string' ? entities.find((e) => e.id === source) : source;
            if (!sourceEntity) {
              return;
            }
            ...
            const targetEntity =
              typeof target === 'string' ? entities.find((e) => e.id === target) : target;
            if (!targetEntity) {
              return;
            }
        ...
        properties.forEach((prop) => {
          entity.getProperty(prop).forEach((value) => {
            ...
            if (prop.type.name === 'entity') {
              const entity =
                typeof value === 'string' ? entities.find((e) => e.id === value) : value;
              if (entity?.id) {
  ...
  getHighlightedEdges(): Edge[] {
    return this.getEdges().filter((edge) => edge.isEntity() && this.isEdgeHighlighted(edge));
  }
  ...
  isGroupingSelected(grouping: Grouping) {
    const selectedVertices = this.selection.filter((vertexId) => this.vertices.has(vertexId));
  ...
  isEdgeAdjacent(edge: Edge, vIds: Array<string>): boolean {
    ...
    return vIds.indexOf(edge.sourceId) !== -1 || vIds.indexOf(edge.targetId) !== -1;
  }
  ...
  applyPositioning(positioningFuncs: any, vertices: Array<Vertex>, maintainFixed = false) {
    ...
    if (maintainFixed) {
      adjEdges = adjEdges.filter(
        (e) => !this.vertices.get(e.sourceId)?.fixed || !this.vertices.get(e.targetId)?.fixed
      );
    }
